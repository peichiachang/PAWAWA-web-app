...
| 錄音 | 本 App 無錄音／語音功能，**不**宣告 RECORD_AUDIO | app.json 不列此權限；若未來新增語音功能需補上並說明用途 |
...
- 食物記錄（FeedingModal）、飲水（HydrationModal）、排泄（EliminationModal）、血檢（BloodReportModal）、用藥（MedicationModal）、症狀（SymptomModal）、設定（SettingsModal）、新增/編輯貓（AddCatModal）、食碗校準（VesselCalibrationModal；不包含測距儀功能，已移除）、飼料設定（FeedLibraryModal）、罐頭庫（CanLibraryModal）、紀錄詳情、血檢歷史／詳情、體重、Kcal/Water 建議、備份、IAP 等，依 PRD 與現有實作。
...
| 1.1 | 2026-03-07 | §6.2 Android：明訂本 App 無錄音／語音功能，不宣告 RECORD_AUDIO；若未來新增需補上並說明用途 |
| 1.2 | 2026-03-07 | §7.3：食碗校準（VesselCalibrationModal）不包含測距儀功能，相關功能與說明已移除 |
